...
def _dir_creator(directory):
    if not os.path.isdir(directory):
        os.mkdir(directory)

def save_img(img, location, filename):
    save_location = os.path.join(location, filename)
    _dir_creator(location.split('/')[-1])
    plt.imsave(fname=save_location, arr=img, format='png')

def write_text(text, location, filename):
    save_location = os.path.join(location, filename+'.txt')
...
def load_all_poses(pose_dir, instances):
    all_poses=[]
    for instance in instances:
        instance_pose_dir = os.path.join(pose_dir, instance,'poses.txt')
        poses = np.loadtxt(instance_pose_dir, dtype=str)
        poses = [int(x.split('/')[-1].split('.')[0]) for x in poses]
...
def plot_images(images, output_dir, filename):
    for i,img in enumerate(images):
        plt.imsave(fname=f'{output_dir}{filename}_{i}.png', arr=img, format='png')