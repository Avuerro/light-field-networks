...
class SceneInstanceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                ...
                #  img_sidelength=None, ## this can never be None due to line 61
                 num_images=None,
                 cache=None,
                 csv=False):
        ...
        split_id = -1 if self.csv else -2 
        self.instance_name = self.instance_dir.split("/")[split_id] #os.path.basename(os.path.dirname(self.instance_dir))
        if self.has_params:
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "pose_p": self.pose_paths[idx],
            'pose_key': self.pose_paths[idx].split('/')[-1],
            'rgb_key': self.color_paths[idx].split('/')[-1]
        }
...
class SceneInstanceDatasetHDF5(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "intrinsics": self.intrinsics,
            "instance_name": self.instance_name,
            'pose_key': self.pose_keys[idx].split('/')[-1],
            'rgb_key': self.color_keys[idx].split('/')[-1]
        }
        return sample

def get_num_instances_csv(csv_loc):
    data = pd.read_csv(csv_loc)
    instances = len(data['instance_id'].unique())
    return instances
...
def get_instance_datasets_csv(root,
                                csv,
                                sidelen,
    ...
    instances_ = pd.read_csv(csv)
    instances = pd.DataFrame(instances_['instance_id'].drop_duplicates(), columns=['instance_id'])
    instances['root'] = root
    # pdb.set_trace()
    data_df = instances[['root','instance_id']]
    # data_df['path'] =  data_df['root'] + '/'+ data_df['instance_id'] ## normally used this
    data_df['path'] =  data_df['root']+ data_df['instance_id']


    instance_dirs = data_df['path'].to_numpy().tolist()
    if max_num_instances != None:
        instance_dirs = instance_dirs[:max_num_instances]
    ## very memory intensive..

    if specific_instances is not None:
        instances = specific_instances
        instances = [  '../../../Datasets/cars_train/' + x for x in instances]
        if specific_poses is None:
            raise Exception('We need specific poses when you specify instances..')

    # pdb.set_trace()


        all_instances = [SceneInstanceDataset(instance_idx=idx+start_idx, instance_dir=instance_name,
                                                specific_observation_idcs=poses,
                                                img_sidelength=sidelen, num_images=max_observations_per_instance,
                                                cache=cache, csv=True)
                            for idx, (instance_name,poses) in enumerate(zip(instances, specific_poses))]
...
def get_instance_datasets(root, sidelen, max_num_instances=None, specific_observation_idcs=None,
                          cache=None, max_observations_per_instance=None, start_idx=0): ## sidelen cant be non see line 61..
...
def get_instance_datasets_hdf5(root, max_num_instances=None, specific_observation_idcs=None,
                               ...
                               start_idx=0,
                               specific_instances=None,
                               specific_poses=None): ##specific poses is a list of list...
    ...
    def __init__(self, num_context, num_trgt, data_root,
        ...
        if 'hdf5' in data_root:
            self.all_instances = get_instance_datasets_hdf5(data_root, max_num_instances=max_num_instances,
                                                            ...
                                                            specific_poses=specific_poses)
        elif csv_loc is not None:
            print('using csv')
            self.all_instances = get_instance_datasets_csv(data_root, csv=csv_loc, max_num_instances=max_num_instances,
                                                            specific_observation_idcs=specific_observation_idcs,
                                                            cache=cache, sidelen=img_sidelength,
                                                            max_observations_per_instance=max_observations_per_instance,
                                                            start_idx=start_idx, 
