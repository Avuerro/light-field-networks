...
def multiscale_training(train_function, dataloader_callback, dataloader_iters, dataloader_params, **kwargs):
    ...
    ## first train with resolution 64 then with 128 why ??
...
def train(model, dataloaders, epochs, lr, epochs_til_checkpoint, model_dir, loss_fn, steps_til_summary=1,
    ...
    with tqdm(total=len(train_dataloader) * epochs) as pbar:
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                for loss_name, loss in losses.items():
                    ...
                    if (loss_schedules is not None) and (loss_name in loss_schedules):
                        if rank == 0:
                            wandb.log({f'{loss_name}_weight':loss_schedules[loss_name](total_steps),'total_steps':total_steps})
                ...
                if rank == 0:
                    wandb.log({'total_train_loss': train_loss, 'total_steps': total_steps})
                ...
                if not total_steps % steps_til_summary and rank == 0:
                    ...
                    if summary_fn is not None:
                        summary_fn(model, model_input, gt, loss_summaries, model_output, total_steps, 'train_')
                    ...
                    if val_dataloader is not None:
                        ...
                        with torch.no_grad():
                            ...
                            for loss_name, loss in val_losses.items():
                                ...
                                if rank == 0:
                                    wandb.log({f'val_{loss_name}': single_loss, 'total_steps': total_steps})
                            if rank == 0:
                                if val_summary_fn is not None:
                                    val_summary_fn(model, model_input, gt, val_loss_smry, model_output, total_steps, 'val_')
        ...
        torch.save(model.state_dict(),
            os.path.join(checkpoints_dir, 'convergence_model_final.pth'))
        partial_model_dict = {'epoch': epoch,
                        'model_state_dict': model.state_dict(),
                        'loss': train_loss}
        optimizer_dict = {f'optimizer{i}_state_dict':optim for i,optim in enumerate(optimizers)}

        full_model_dict = dict(partial_model_dict, **optimizer_dict)
        torch.save(full_model_dict, os.path.join(checkpoints_dir, 'convergence_checkpoint_model_final.pth'))
